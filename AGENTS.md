...
- **Features**：8 张功能卡片，展示时间线视图、标签、Apple 提醒事项集成、日历同步、快速操作、专注模式、数据洞察、深色模式与本地化。
- **Screenshots**：横向滚动轮播，展示 5 张应用截图；图片资源应通过 `data-src-zh` / `data-src-en` 随网站语言切换。
...
- 首页应合理说明 Chrono 支持深色模式和本地化。截图素材应按语言分目录维护，真实多语言截图未整理好时先使用占位图。
