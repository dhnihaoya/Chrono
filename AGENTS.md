...
- `pics_dark/`：深色模式资源目录，当前只包含深色 App Icon。
...
- **Screenshots**：横向滚动轮播，展示 5 张应用截图；图片资源应通过 `data-src-zh` / `data-src-en` 随网站语言切换。浅/深网页配色切换不应切换截图，等深色截图整理好后再单独接入。
...
- 截图素材应按语言分目录维护。真实多语言截图未整理好时先使用占位图；真实深色截图未整理好前，不要让页面切换截图资源。
